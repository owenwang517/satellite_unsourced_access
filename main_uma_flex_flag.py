...
for idx_sim in range(num_sim):
    ...
    act_code_num_true = np.zeros(args.num_seg,dtype=np.int32)
    for idx_seg in range(args.num_seg):
        act_code_num_true[idx_seg] = len(set(message[idx_seg]))
    if np.max(act_code_num_true) < args.num_aue:
        count_collide += 1
    ...
    channel_mtx = torch.tensor(channel_mtx, dtype=torch.complex64).cuda()
    channel_mtx = channel_mtx.reshape((args.num_seg, args.nc,args.num_blk, num_code, args.Nx*args.Ny))
    channel_mtx = channel_mtx[:,0,:,:,:]
    ce_nmse = (channel_spa_hat - channel_mtx).abs().square().sum()/channel_mtx.abs().square().sum()
    ...
    ce_nmse = 10 * torch.log10(ce_nmse.cpu())
    ...
    print('sim = %6d, code error prob= %6.6f, stitch error prob = %6.6f, average pe = %6.6f, collide_prob = %6.6f, NMSE = %6.6f' % (idx_sim, pec, pes, pes_sum/(idx_sim+1), count_collide/(idx_sim+1),ce_nmse))
assert False